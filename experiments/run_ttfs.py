...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[TTFS] Epoch [{epoch}/{epochs}] "
    ...
    print(f"\n[TTFS] Final Test Loss: {test_loss:.4f} | Final Test Acc: {test_acc*100:.2f}% | "
          f"{metrics_to_str(test_m)}")
    if "median_latency_t" in test_m:
        print(f"[TTFS] Median Lat: {test_m['median_latency_t']:.2f} | "
              f"Early Decision Rate: {test_m['early_decision_rate']*100:.2f}%")

    # Robustness: Gaussian noise
    print("\n[TTFS] Robustness (Gaussian noise)")
